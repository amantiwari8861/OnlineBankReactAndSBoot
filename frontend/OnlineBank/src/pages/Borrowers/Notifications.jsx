//http for borrower notification for post :  http://localhost:5000/api/borrower-notification
...
const Notifications = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(import.meta.env.VITE_API_URL+`/borrower-notification/${userId}`);
  ...
  const handleMarkAsRead = async (notificationId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(import.meta.env.VITE_API_URL+`/borrower-notification/${notificationId}`, 
      { status: 'read' }, 
